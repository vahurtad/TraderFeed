...
var GDAXExchangeAPI_1 = require("../../src/exchanges/gdax/GDAXExchangeAPI");
var assert = require('assert');
var nock = require('nock');
describe('GDAX Exchange API', function () {
    var gdax;
    before(function () {
        gdax = new GDAXExchangeAPI_1.GDAXExchangeAPI({ logger: null });
    });
    it('returns a ticker object', function () {
        ...
        return gdax.loadTicker('BTC-USD').then(function (ticker) {
            assert(ticker.bid.eq(249));
            assert(ticker.ask.eq(250));
            assert(ticker.volume.eq(1000));
        });
    });
    it('returns a midmarket price', function () {
        nock('https://api.gdax.com', { encodedQueryParams: true })
            .get('/products/BTC-USD/ticker')
            .reply(200, {
            ask: '250.0',
        ...
        return gdax.loadMidMarketPrice('BTC-USD').then(function (price) {
            assert(price.eq(249.5));
        });
    });
    it('loads the orderbook', function () {
        nock('https://api.gdax.com', { encodedQueryParams: true })
        ...
        return gdax.loadOrderbook('BTC-USD').then(function (book) {
    ...
    it('returns candles', function () {
        ...
        return gdax.loadCandles({ gdaxProduct: 'BTC-USD', interval: '1h', limit: 3, from: null, extra: null }).then(function (candles) {
...
describe('GDAX Authenticated Exchange API', function () {
    var gdax;
    before(function () {
        gdax = new GDAXExchangeAPI_1.GDAXExchangeAPI({ logger: null, auth: { key: 'key', secret: 'secret', passphrase: 'pass' } });
    });
    it('handles errors from the gdax module', function () {
        nock('https://api.gdax.com', { encodedQueryParams: true })
            .get('/products')
            .reply(400, { reason: 'Fake failure' });
        return gdax.loadProducts().then(function (products) {
            return Promise.reject("Unexpected success " + products);
        }, function (err) {
            assert(err);
            assert(err.response);
            assert.equal(err.response.status, undefined);
            assert.equal(err.response.body, JSON.stringify({ reason: 'Fake failure' }));
        });
    });
    it('handles errors when it does API calls', function () {
        nock('https://api.gdax.com', { encodedQueryParams: true })["delete"]('/orders/foobar')
            .reply(400, { reason: 'Fake failure' });
        return gdax.cancelOrder('foobar').then(function (id) {
    ...
    it('returns a crypto address', function () {
        nock('https://api.gdax.com', { encodedQueryParams: true })
        ...
        return gdax.requestCryptoAddress('BTC').then(function (result) {
